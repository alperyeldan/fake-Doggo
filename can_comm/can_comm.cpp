...
void receive(CAN& can_interface,RecievedData& rcvd){
    ...
    if(can_interface.read(msg)) {
        ...
        rcvd.u_position = position;
        rcvd.u_velocity = velocity;
        rcvd.u_current  = current;
        rcvd.u_ID = ID;

        rcvd.f_position = uint_to_float(position, P_MIN, P_MAX, 16);
        rcvd.f_velocity = uint_to_float(velocity, V_MIN, V_MAX, 12);
        rcvd.f_current  = uint_to_float(current,  I_MIN, I_MAX, 12);
        rcvd.f_ID = (float)ID;
    }
}
